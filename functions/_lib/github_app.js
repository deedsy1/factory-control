...
export async function getGithubContents(env, ownerRepo, path, ref = "main") {
  const url = `https://api.github.com/repos/${ownerRepo}/contents/${encodeURIComponent(path).replace(/%2F/g, "/")}?ref=${encodeURIComponent(ref)}`;
  const res = await ghFetch(env, url, { method: "GET" });
  const text = await res.text();
  let data = null;
  try { data = JSON.parse(text); } catch {}
  if (!res.ok) {
    const msg = (data && data.message) ? data.message : text || `GitHub GET contents failed (${res.status})`;
    throw new Error(msg);
  }
  return data;
}

export async function putGithubContents(env, ownerRepo, path, contentText, message, sha = null, branch = "main") {
  const url = `https://api.github.com/repos/${ownerRepo}/contents/${encodeURIComponent(path).replace(/%2F/g, "/")}`;
  const body = {
  ...
  try { data = JSON.parse(text); } catch {}
  if (!res.ok) {
    const msg = (data && data.message) ? data.message : text || `GitHub PUT contents failed (${res.status})`;
    throw new Error(msg);
  }
  return data;
}
